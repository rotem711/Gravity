...
const HeadlineCopyMediaRowsModule: FunctionComponent<IHeadlineCopyMediaRows> = (
  props,
) => {
  ...
  return (
    ...
    >
      <div className="container default-grid">
        <h2 className="col-span-6 md:col-span-12 typo-subhead uppercase mb-110 lg:mb-155">
          <Fade>{headlineCopyMediaRows.topHeadline}</Fade>
        </h2>
        {headlineCopyMediaRows.rows.map((item, index) => (
          ...
          >
            ...
            <div
              className={`col-span-6 md:col-start-10 md:col-end-13 mt-10 md:mt-0 relative ${styles.mediaContainer}`}
            >
              {item.vimeoVideoUrl && inView ? (
                <video
                  key={item.vimeoVideoUrl}
