...
const HeaderBlock = ({ data, inverted }: HeaderInterface) => {
  ...
  return (
    <header className={`${styles.root} ${deployed && styles['is-deployed']} ${inverted ? styles['is-inverted'] : ''} container flex items-center justify-between pt-30 pb-30 xl:pt-35 xl:pb-35 bg-white xl:bg-opacity-0`} data-scroll-dir={scrollDir}>
      <Link href="/">
        <a className={`${styles.logo}`}><Logo /></a>
      </Link>
      <div className={`${styles.navContainer} ${deployed && styles['is-deployed']}`}>
          ...
          <nav>
            <ul className="xl:flex">
              {data.rightSideNavigation.map((subItem) => (
                <li key={subItem.link.title}>
                  <Link href={subItem.link.url}>
                    <a className={`${styles.navItem} ${subItem.link.title === 'Get Started' ? 'hidden xl:block' : ''} last:mr-0`} target={subItem.link.target}>
                      {subItem.link.title}
                    </a>
                  </Link>
                </li>
      ...
      <div className="xl:hidden z-10">
        <button type="button" className={`${styles.navSubItem} mr-0`} onClick={() => setDeployed(!deployed)}>
